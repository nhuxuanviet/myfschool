...
```
/api/v1/student/**    chuyển từ /api/v1/home, /timetable, /grades, /events,
                      /forms, /clubs, /assistant
/api/v1/teacher/**    mới
/api/v1/parent/**     mới
/api/v1/admin/**      giữ nguyên
```

Việc chuyển đường dẫn học sinh là thay đổi phá vỡ với ứng dụng Flutter và 11 bộ
kiểm thử đầu-cuối đang xanh. Nó được tách thành một bước riêng, thuần cơ học,
chạy toàn bộ hồi quy ngay sau đó, để rủi ro nằm gọn trong R1.
...
| Rủi ro | Xử lý |
| --- | --- |
| Chuyển API học sinh sang `/student/**` phá vỡ 11 bộ E2E đang xanh | Tách thành bước riêng, thuần cơ học, chạy full hồi quy ngay sau |
...
| --- | --- | --- |
| R1 | `user_roles`, hồ sơ giáo viên và phụ huynh, liên kết giám hộ, JWT gắn vai, đổi vai, chuyển API học sinh, Admin quản lý giáo viên/phụ huynh/liên kết | Giáo viên và phụ huynh không gọi được API của nhau; hồi quy học sinh xanh sau khi đổi đường dẫn |
